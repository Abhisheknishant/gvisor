...
func (ndp *ndpState) sendDADPacket(addr tcpip.Address, ref *referencedNetworkEndpoint) *tcpip.Error {
	...
	if c, err := r.Resolve(nil); err != nil {
		// Do not consider the NIC being unknown as a fatal error. Since this method
		// is required to be called when the NIC is not locked, the NIC could have
		// been removed by another goroutine.
...
func (ndp *ndpState) startSolicitingRouters() {
	...
	ndp.rtrSolicit.timer = time.AfterFunc(delay, func() {
		...
		// remote link address can be calculated without a resolution process.
		if c, err := r.Resolve(nil); err != nil {
			// If the NIC is unrecognized when we attempt to resolve the link address,
			// that means the NIC was removed after we released the lock but before
			// we reached this point. Do not consider this a fatal error and do
			// nothing further.
			if err == tcpip.ErrUnknownNICID {
				return
			}

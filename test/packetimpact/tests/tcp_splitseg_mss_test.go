...
func TestTCPSplitSegMSS(t *testing.T) {
	...
	p, err := conn.ExpectData(&tb.TCP{Flags: tb.Uint8(header.TCPFlagAck | header.TCPFlagPsh)}, &unexpectedPayload, 3*time.Second)
	if err == nil {
		// Expect the payload to be after tcp, <ether><ip><tcp><payload>.
		const payloadLayer = 3
		if got := p[payloadLayer].(*tb.Payload); got.Length() > int(mss) {
			t.Fatalf("unexpected TCP payload larger than MSS was received %s", got)
		}
	}
}
